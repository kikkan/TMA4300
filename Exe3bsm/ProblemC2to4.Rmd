...
In this task, the standard deviations and biases of $\hat{\lambda_0}$ and $\hat{\lambda_1}$ in addition to $\text{corr} [\hat{\lambda_0}, \hat{\lambda_1}]$ are estimated by using bootstrap. The pseudocode for the bootstrap algorithm is presented below.
...
The algorithm is then implemented
...
l.var.boot = var(lambda.T)
var.boot = sqrt(diag(l.var.boot))
