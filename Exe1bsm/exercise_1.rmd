...
$$
If the prior has a $\text{beta}(1,5)$ distribution, the posterior is 
$$
f(\theta|y) \propto \frac{(1-\theta)^{\beta-1}/B(1,5) (2+\theta)^{y_1}(1-\theta)^{y_2+y_3} \theta^{y_4}}{\int_{0}^{1}(2+\theta)^{y_1}(1-\theta)^{y_2+y_3}\theta^{y_4}(1-\theta)^{\beta-1}/B(1,5) d\theta}
$$
$$
\propto (1-\theta)^{\beta-1}/B(1,5) (2+\theta)^{y_1}(1-\theta)^{y_2+y_3} \theta^{y_4}
$$
\textcolor{green}{D4 chris} 

In section [D2](#D2) we used a uniform prior, i.e Beta$(1,1)$ prior for $\theta$. Now we will assume a Beta$(1,5)$ prior instead, that is
$$
\begin{split}
...
\textcolor{blue}{add fab stuff above if needed?}

```{r D4beta15prior, options}
posterior15 <- function(theta, normalize){
  # Posterior when prior is beta(1,5)
  return((2 + theta)^125 * (1 - theta)^(18 + 20 + 4) * theta^(34)/normalize)
}

norm_con15 = integrate(posterior15, 0, 1)$val
theta_sampling15 = rejection_sampling_f(M, posterior15)
```
