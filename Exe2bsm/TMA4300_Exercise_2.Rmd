...
mcmcIndivid = function(N, dt, sigma0=0.05){
  ...
  for (i in 2:N){
    ...
    mhStep = mhFirst(tau_i, sigma_i, dt$n.rain[1], 1, normMat[i,1], unifMat[i,1])
    tau_i[1] = mhStep$tau
    # tau[i,1] = mhStep$tau
    ...
    for (t in 2:(Ttot-1)){
      ...
      # ratio = acceptRatio(n.years[t], n.rain[t], prop_tau, tau_i[t])
...
lines(rain$day,pSeq[N,], 
      # type = "l",ylab=expression(paste(pi,"(",tau[366],")")),
...
pSeqBlock2 = apply(resultList[[1]], 2, link)
pSeqBlock5 = apply(resultList[[3]], 2, link)
pSeqBlock10 = apply(resultList[[5]], 2, link)
pSeqBlock20 = apply(resultList[[7]], 2, link)
```
